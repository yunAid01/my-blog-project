...
export const createFollow = async  (followedId: number): Promise<createFollowReturn> => {
...
export const deleteFollow = async  (unfollowedId: number): Promise<deleteFollowReturn> => {
    const API_URL=process.env.NEXT_PUBLIC_API_URL
    const token = localStorage.getItem('jwt-token');
    if (!token) {
        alert('로그인 필요')
