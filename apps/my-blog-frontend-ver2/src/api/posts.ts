...
import type { GetPostReturn, UpdatePostData, CreatePostData, PostForMainPage, Post } from "@my-blog/types";
...
export const updatePost = async ({ postId, updatePostData } : UpdatePost): Promise<Post> => {
    const post: Post = await apiClient.patch(`/posts/${postId}/edit`, updatePostData)
    return post 
}
...
export const deletePost = async (postId: number): Promise<DeletePostReturn> => {    
    const deletePostReturn: DeletePostReturn  = await apiClient.delete(`/posts/${postId}`)
